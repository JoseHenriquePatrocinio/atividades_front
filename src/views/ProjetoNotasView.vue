<template>
    <body>
        <button class="add" @click="addNewNote">
            Add note
        </button>

        <div v-for="(note, index) in notes" :key="index" class="note">
            <div class="tools">
                <button class="edit" @click="toggleEdit(index)">
                    <svg-icon type="mdi" :path="mdiCheckBold"></svg-icon>
                </button>
                <button class="delete" @click="deleteNote(index)">
                    <svg-icon type="mdi" :path="mdiTrashCan"></svg-icon>
                </button>
            </div>

            <div class="main" v-if="!note.editing">{{ note.text }}</div>
            <textarea v-if="note.editing" v-model="note.text" @input="updateNote"></textarea>
        </div>
    </body>
</template>
  
<script>
import SvgIcon from '@jamescoyle/vue-icon';
import { mdiTrashCan } from '@mdi/js';
import { mdiCheckBold } from '@mdi/js';

export default {
    name: "ProjetoNotasView",
    components: {
        SvgIcon
    },
    data() {
        return {
            mdiTrashCan: mdiTrashCan,
            mdiCheckBold: mdiCheckBold,
            notes: JSON.parse(localStorage.getItem('notes')) || []
        };
    },
    methods: {
        addNewNote() {
            this.notes.push({ text: "", editing: true });
            this.updateLS();
        },
        toggleEdit(index) {
            this.notes[index].editing = !this.notes[index].editing;
            this.updateLS();
        },
        deleteNote(index) {
            this.notes.splice(index, 1);
            this.updateLS();
        },
        updateNote() {
            this.updateLS();
        },
        updateLS() {
            localStorage.setItem('notes', JSON.stringify(this.notes));
        }
    }
};
</script>
<style scoped>
body {
    background-color: #091974;
    font-family: 'Poppins', sans-serif;
    display: flex;
    flex-wrap: wrap;
    margin: 0;
    padding-top: 3rem;
    min-height: 85vh;
}

.add {
    position: fixed;
    top: 3.5rem;
    right: 1rem;
    background-color: #9ec862;
    color: #fff;
    border: none;
    border-radius: 3px;
    padding: 0.5rem 1rem;
    cursor: pointer;
}

.add:active {
    transform: scale(0.98);
}

.note {
    background-color: #fff;
    box-shadow: 0 0 10px 4px rgba(0, 0, 0, 0.1);
    margin: 30px 20px;
    height: 400px;
    width: 400px;
    overflow-y: scroll;
}

.note .tools {
    background-color: #9ec862;
    display: flex;
    justify-content: flex-end;
    padding: 0.5rem;
}

.note .tools button {
    background-color: transparent;
    border: none;
    color: #fff;
    cursor: pointer;
    font-size: 1rem;
    margin-left: 0.5rem;
}

.note textarea {
    outline: none;
    font-family: inherit;
    font-size: 1.2rem;
    border: none;
    height: 400px;
    width: 100%;
    padding: 20px;
}

.main {
    padding: 20px;
}

.hidden {
    display: none;
}
</style>